 ...
 *
 *******************************************************************************/
package ai.aitia.arrowhead.it2generichttp.http.filter.authorization;
...
public class ManagementFilter extends ArrowheadFilter {
	...
	private boolean checkToken(final HttpServletRequest request) {
		...
		final String initVector = arrowheadContext.get(InterfaceTranslatorToGenericHTTPConstants.KEY_INITIALIZATION_VECTOR).toString();
		try {
	...
	private String decrypt_AES_CBC_PKCS5P_IV(final String encryptedDataBase64, final String ivBase64, final String key)
		...
		final Cipher cipher = Cipher.getInstance(InterfaceTranslatorToGenericHTTPConstants.AES_CBC_ALGORITHM_IV_BASED);
